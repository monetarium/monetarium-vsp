...
func TestSetAltSignAddress(t *testing.T) {
	// Monetarium mainnet P2PKH (hash160 = 0x01..0x14): the upstream fixture was
	// a Decred "Ds" address, which fails stdaddr decoding under Monetarium
	// params and turned every subtest into a 400 before reaching the mocked
	// clients.
	...
	tests := map[string]struct {
		...
		"addr wrong type": {
			addr:           "McTDNPRfQPC2KYNQPquGH1wLZXB1wQFZhT3",
			wantHTTPStatus: http.StatusBadRequest,
			wantErrCode:    types.ErrBadRequest,
